...
# 用于在 Sub-Agent 工具包装器中构造输入消息，将自然语言请求传入 Sub-Agent
...
#   用于在 Sub-Agent 工具中获取 user_id 等上下文，替代 EP14 的 ToolRuntime[Context] 方式
...
async def get_knowledge_agent_tools():
    ...
    # EP15：MCP 地址从 Config 读取，便于切换主机/端口（EP14 为硬编码 127.0.0.1:8010）
    ...
    # 构建一个工具中断配置字典：search_documents 需要人工审批（与 EP14 get_tools 中策略一致，前缀标明所属 Agent）
    interrupt_on = {
        'search_documents': {
            'allowed_decisions': ['approve', 'edit', 'reject'],
...
async def get_task_agent_tools():
    ...
    无需 context_schema；Supervisor 工具包装器调用 sub_agent.ainvoke() 时会
    显式透传 configurable（含 user_id），因此本工具可正确读取用户上下文。
    """
    ...
    # EP14 使用 ToolRuntime[Context] 读取 user_id；EP15 改为 get_config()，与 Agents as Tools 的 configurable 透传一致
    ...
    async def get_user_location() -> str:
        ...
        Supervisor 工具包装器在 ainvoke 时显式传入 configurable，保证此处可读取。
        """
...
# ──────────────────────────────────────────────────────
#  Supervisor 工具集：将各 Sub-Agent 包装为高层工具（Agents as Tools 模式）
# ──────────────────────────────────────────────────────
def get_supervisor_tools(web_agent, knowledge_agent, task_agent):
    """
    将三个 Sub-Agent 包装为 Supervisor 可调用的工具（Agents as Tools 模式）。

    这是官方 langchain.com/oss/python/langchain/multi-agent/subagents-personal-assistant
    文档中描述的架构：Supervisor 通过高层工具（schedule_event / manage_email 等）
    委派任务给 Sub-Agent，Sub-Agent 以自然语言请求为输入，返回自然语言结果字符串。

    configurable（含 user_id / thread_id）通过 get_config() 从 Supervisor 的
    运行时配置中透传给 Sub-Agent，保证 get_user_location 等工具能正确读取上下文。

    HITL 行为：Sub-Agent 内部的 HumanInTheLoopMiddleware 会触发中断，
    该中断会通过 Sub-Agent → 工具函数 → Supervisor 这条调用链向上传播；
    只有 Supervisor 拥有 checkpointer，因此中断状态由 Supervisor 层统一保存与恢复。
    """

    @tool(
        "web_agent_tool",
        description=(
            "使用真实浏览器浏览网页并提取内容，适用于：打开指定 URL、读取页面文本/链接、"
            "点击页面元素等浏览器操作。"
            "输入：自然语言网页访问指令（例如 '打开 https://example.com 并告诉我主要内容'）。"
            "注意：navigate_browser 和 click_element 默认需要人工审批。"
        ),
    )
    async def web_agent_tool(request: str) -> str:
        # 透传 Supervisor 的 configurable（thread_id / user_id）给 Sub-Agent
        cfg = get_config() or {}
        result = await web_agent.ainvoke(
            {"messages": [HumanMessage(content=request)]},
            config={"configurable": cfg.get("configurable", {})},
        )
        # 返回 Sub-Agent 最后一条消息内容（字符串），供 Supervisor 作为工具结果继续推理
        return result["messages"][-1].content

    @tool(
        "knowledge_agent_tool",
        description=(
            "在向量知识库中进行语义检索，适用于：查找文档资料、知识问答、"
            "搜索与某主题相关的内容。"
            "输入：自然语言检索请求（例如 '在知识库中查找关于 RAG 的文章'）。"
        ),
    )
    async def knowledge_agent_tool(request: str) -> str:
        cfg = get_config() or {}
    ...
    @tool(
        "task_agent_tool",
        description=(
            "执行实用任务，适用于：查询城市天气、获取用户当前位置、"
            "文本摘要/翻译/统计分析等技能执行。"
            "输入：自然语言任务指令（例如 '查询北京今天的天气' 或 '帮我总结以下文字…'）。"
        ),
    )
    async def task_agent_tool(request: str) -> str:
        cfg = get_config() or {}
    ...
    # Supervisor 仅持有这三个高层工具，由 LLM 根据描述选择路由到对应 Sub-Agent
    tools = [web_agent_tool, knowledge_agent_tool, task_agent_tool]
    logger.info(f"Supervisor 工具列表（Agents as Tools）: {[t.name for t in tools]}")
    return tools
