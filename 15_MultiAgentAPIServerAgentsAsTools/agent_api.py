...
# 导入 LangChain 的 create_agent，用于创建 Supervisor Agent
from langchain.agents import create_agent
# 导入项目配置类
from utils.config import Config
# 导入获取 LLM 的函数
from utils.llms import get_llm
# 导入 Playwright 浏览器句柄管理函数与 Supervisor 工具包装函数
from utils.tools import (
    set_async_playwright_browser,
    clear_async_playwright_browser,
    get_supervisor_tools
)
# 导入请求/响应模型与上下文模型（AskRequest、InterveneRequest、AgentResponse、Context）
from utils.models import AskRequest, InterveneRequest, AgentResponse, Context
# 导入日志管理器
...
# 声明全局变量：检查点保存器（短期记忆，只有 Supervisor 持有）
...
# supervisor_agent：create_agent(tools=[web_agent_tool, knowledge_agent_tool, task_agent_tool])
# 只有 Supervisor 拥有 checkpointer；Sub-Agent 被包装为工具，每次工具调用时独立执行
...
async def lifespan(app: FastAPI):
    ...
    启动阶段：
      ...
      5. 用 get_supervisor_tools 包装 Sub-Agent 为工具，
         再用 create_agent 创建 Supervisor（只有 Supervisor 持有 checkpointer）

    关闭阶段：释放 Playwright、关闭连接池
    """
    # 声明使用全局变量 pool、checkpointer、store、playwright_instance、supervisor_agent
    global pool, checkpointer, store, playwright_instance, supervisor_agent
    ...
    # 6. 将 Sub-Agent 包装为高层工具，创建 Supervisor Agent
    #
    #  Agents as Tools 核心设计：
    #  ① get_supervisor_tools 创建 web_agent_tool / knowledge_agent_tool / task_agent_tool
    #     每个工具内部调用 sub_agent.ainvoke()，返回 Sub-Agent 最后一条消息内容（字符串）
    #  ② Supervisor LLM 通过选择工具来"路由"任务
    #  ③ 只有 Supervisor 持有 checkpointer 和 store（Sub-Agent 无状态）
    #  ④ HITL 中断从 Sub-Agent 内部工具触发，经工具包装器传播至 Supervisor，
    #     由 Supervisor 的 checkpointer 捕获和保存
    #  ⑤ 支持并行工具调用（LLM 同时调用多个 Sub-Agent 工具）
    #
    ...
    # 获取 Sub-Agent 工具包装器列表（web_agent_tool / knowledge_agent_tool / task_agent_tool）
    supervisor_tools = get_supervisor_tools(web_agent, knowledge_agent, task_agent)

    # 创建 Supervisor Agent：只有 Supervisor 持有 checkpointer 和 store
    supervisor_agent = create_agent(
        model=llm_chat,
        system_prompt=supervisor_prompt,
        tools=supervisor_tools,
        # 只有 Supervisor 持有 checkpointer：HITL 中断状态在此层保存，
        # 恢复时通过 Command(resume=...) 找到正确的断点继续执行
        checkpointer=checkpointer,
        store=store,
    )
    logger.info("Supervisor Agent 创建完成（Agents as Tools），端口: %s", Config.API_SERVER_PORT)
...
app = FastAPI(
    title="Multi-Agent API（官方 Agents as Tools 模式）",
    description=(
        "基于 LangChain 官方文档 subagents-personal-assistant 的多智能体 API 服务：\n"
        "Supervisor 通过工具包装器（Agents as Tools）委派任务给 Web / Knowledge / Task Agent，\n"
...
def _extract_hitl_details(interrupt_value: Any) -> Dict[str, Any]:
    """
    在 Agents as Tools 模式下，HITL 中断从 Sub-Agent 内部工具触发，
    经工具包装器向上传播至 Supervisor。中断格式统一来自
    HumanInTheLoopMiddleware（action_requests / review_configs）。
    """
...
# 定义异步函数：非流式执行 supervisor_agent 并处理 HITL 中断，返回状态字典
async def run_with_hitl(
    user_content: str,
    config: dict,
) -> Dict[str, Any]:
    """非流式执行 supervisor_agent，处理 HITL 中断。"""
    # 写入一条长期记忆到存储，实际项目可按需修改此处内容与 user_id
    await write_long_term_info("user_001", "南哥")

    # 调用 supervisor_agent 的 ainvoke 方法，参数为初始消息和运行配置
...
# 定义异步函数：流式执行 supervisor_agent，遇到 HITL 中断则 yield interrupted 并结束
async def run_streaming(
    user_content: str,
    config: dict,
) -> AsyncGenerator[Dict[str, Any], None]:
    """
    流式执行 supervisor_agent，推送 SSE 事件：
    - token：Supervisor 或 Sub-Agent 的 LLM 输出片段
    - tool_output：Sub-Agent 工具包装器返回的结果字符串
    - interrupted：HITL 中断（Sub-Agent 内部工具触发，向上传播至 Supervisor）
    - completed：全部执行完毕
    """
    # 使用 supervisor_agent.astream 进行异步流式输出，含 updates 和 messages 两种流类型
    async for stream_mode, chunk in supervisor_agent.astream(
        {"messages": [{"role": "user", "content": user_content}]},
        config=config,
        ...
        if stream_mode == "messages":
            ...
            if not content:
                continue
            # 取 node 名称以判断是否为工具节点输出（Sub-Agent 工具包装器返回）
    ...
    # 全部流程结束后，主动获取当前 supervisor_agent state
...
# 定义异步函数：提交 HITL 决策后流式恢复 supervisor_agent 的执行
async def resume_streaming(
    config: dict,
    decisions: list,
    interrupt_id: str | None = None,
) -> AsyncGenerator[Dict[str, Any], None]:
    """提交 HITL 决策，流式恢复 supervisor_agent 的执行。"""
    # 调用 supervisor_agent 的 astream 方法，参数为恢复执行的决策及配置
    async for stream_mode, chunk in supervisor_agent.astream(
    ...
    # 全部流程结束后，获取当前 supervisor_agent 的 state
    state = await supervisor_agent.aget_state(config)
