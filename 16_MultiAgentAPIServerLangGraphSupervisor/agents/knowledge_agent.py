...
async def build_knowledge_agent(llm_chat):
    ...
    - name="knowledge_agent" 是关键：create_supervisor 据此在 StateGraph 中注册节点
      并自动生成 transfer_to_knowledge_agent Handoff 工具，供 Supervisor 路由时调用
    ...
    agent = create_agent(
        ...
        # 必填：Supervisor 据此注册 StateGraph 节点并生成 transfer_to_knowledge_agent Handoff 工具
        name="knowledge_agent",
    )
    # 记录 Knowledge Agent 构建完成日志
    logger.info("Knowledge Agent 构建完成（name=knowledge_agent）")
    return agent
